...
import {
  GET_PROJECT_NODE_TYPES_LIST_LEFT_MENU,
...
export const NodeTypes = ({ searchVisible, setSearchVisible, isCheckable = false, noColors = false }: Props) => {
  ...
  const {
    ...
    {
      url: isPublicPage ? GET_PUBLIC_PROJECT_NODE_TYPES_LIST : GET_PROJECT_NODE_TYPES_LIST_LEFT_MENU,
      projectId: params.id || '',
    },
    {
      ...
      onSuccess: (data) => {
        ...
        localStorage.setItem('type_id', data?.data[0]?.id);
