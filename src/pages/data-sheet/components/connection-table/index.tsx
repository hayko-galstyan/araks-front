...
import { convertByType, extractTextFromHTML } from 'helpers/utils';
    ...
    onSuccess: ({ rows: data }) => {
      const rows = data.map((row) => ({
        ...
        ...row.properties?.reduce((curr: DataType, item: EdgesCreateProperties) => {
          const value = convertByType(item.data, item.edge_type_property_type, true);
          const processedValue =
            typeof value === 'string' && value.includes('<p>') ? extractTextFromHTML(value) : value;

          return {
            ...curr,
            [item.edge_type_property_id]: processedValue,
          };
        }, {} as DataType),
      }));
    ...
    <div style={{ position: 'relative' }}>
      ...
      >
        ...
        <ConnectionTable
          ...
          pagination={false}
          scroll={{ x: 'max-content', y: `calc(100vh - ${(isXXl ? 152 : 130) + 245}px)` }}
        />
      </div>
      {pageCount && (
        <NodePagination
