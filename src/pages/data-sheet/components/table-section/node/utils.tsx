...
const showText = (
  ...
  switch (propertyType) {
    ...
    case PropertyTypes.URL:
      const urlWithProtocol = text.startsWith('http://') || text.startsWith('https://') ? text : `http://${text}`;
      return (
        <Button type="link" href={`${urlWithProtocol}`} target="_blank" rel="noopener noreferrer">
          {text}
        </Button>
      );
...
const dataByType = (nodeData: NodeDataType, propertyType: PropertyTypes) => {
  ...
  switch (propertyType) {
    ...
    case PropertyTypes.URL:
      const urlWithProtocol = text.startsWith('http://') || text.startsWith('https://') ? text : `http://${text}`;
      return (
        <Button type="link" href={urlWithProtocol} target="_blank" rel="noopener noreferrer" icon={<LinkOutlined />}>
          {text}
        </Button>
      );
...
export const setNodeDataValue = (item: ProjectTypePropertyReturnData, values: NodeBody) => {
  ...
  if (item.ref_property_type_id === PropertyTypes.Document) {
    return (values[item.name] as UploadFile[]).map((item) => ({
      ...
      mimetype: item?.response?.data.mimetype,
      created_at: item?.response?.data.created_at,
    }));
  }
