...
import { Avatar, Col, List, Row, Skeleton, Spin, Tooltip } from 'antd';
...
export const AllDataListNode = ({
  ...
  return (
    <Spin spinning={isInitialLoading}>
      ...
      <List
        style={{ overflow: 'auto', height: `calc(100vh - ${(isXXl ? 152 : 130) + 208}px)` }}
        pagination={false}
        dataSource={rowsData}
            ...
            <StyledListItem key={item.id} color={item.nodeType?.color}>
              ...
              <List.Item.Meta
                ...
                title={
                  <Row align="middle">
                    <Col span={6}>
                      <Button className="default-name-btn" onClick={() => dispatch(item.id)} type="link">
                        <Tooltip title={item.name} placement="bottom">
                          <Text className="default-name" color={COLORS.PRIMARY.GRAY}>
                            {item.name}
                          </Text>
                        </Tooltip>
                      </Button>
                    ...
                    <Col span={18}>
                      <Row justify="end" align="middle" gutter={32}>
                        <Col span={6}>
                          {item?.documents !== undefined && item.documents > 0 && (
                            <Row gutter={8} align="middle">
                              <Col>
                                <span style={{ color: '#232F6A' }}>{item?.documents}</span>
                              </Col>
                              <Col>
                                <FileTextOutlined style={{ color: '#232F6A' }} />
                              </Col>
                            </Row>
                          )}
                        </Col>
