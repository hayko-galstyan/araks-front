...
export const Styling = () => {
  ...
  const updateGraphItems = (query: any, filteredNodes: INode[]) => {
  ...
  const handleNodeFilterAndUpdate = (query: any) => {
    if (query.parent_id && query.depth === 2) {
      ...
      updateGraphItems(query, filteredNodes);
    } else {
      ...
      removedNodes.forEach((node) => {
        updateGraphItems(query, [node]);
      });
    }
  };

  const handleEdgeFilterAndUpdate = (query: any) => {
  ...
  const onFinish = (values: Props) => {
    if (values.queries) {
      ...
      values.queries.forEach((query) => {
        handleNodeFilterAndUpdate(query);
        handleEdgeFilterAndUpdate(query);
      });
    }
  };
