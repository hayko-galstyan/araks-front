...
export const useGraphRef: Props = ({ nodes, edges, count, relationsCounts, callpased }) => {
  ...
  useEffect(() => {
    if (graph) {
      const timeoutId = setTimeout(() => {
        const newWidth = callpased ? window.innerWidth - callpased : window.innerWidth;
        graph.changeSize(newWidth, graph.getHeight());
        graph.fitCenter();
        graph.fitView();
      }, 100);
