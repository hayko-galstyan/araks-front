...
function ViewDatasheetProvider({ children }: ViewDatasheetProviderProps) {
  ...
  const [isView, setIsView] = React.useState({ jira: false, edit: false });
  ...
  const onFinish = (values: NodeBody) => {
    const mainData = {
      ...
      default_image:
        values.default_image && (values.default_image as [])?.length > 0
  ...
  return (
    <ViewDatasheetContext.Provider value={value}>
      ...
      <Drawer
        ...
        afterOpenChange={(open) => {
          if (!open) {
            ...
            setSelectedViewId(undefined);
            setIsView({ jira: false, edit: false });
            form.resetFields();
          }
        }}
        open={!!selectedViewId}
      ...
      >
        <>
          {isView.jira ? (
            <ToolSetJira handleBackClick={() => setIsView({ jira: false, edit: false })} />
          ) : (
            <>
              <DocumentDrawerWrapper className="document-drawer" />
