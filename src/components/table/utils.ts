...
type TGenereateNewRowPropertyParam = {
  user_id: string | undefined;
  project_id: string | undefined;
  project_type_id: string | undefined;
  name: string;
  type: string;
  key: string;
};
...
export function generateNewRowProperty(data: TGenereateNewRowPropertyParam) {
  return {
    created_at: '',
    ...
    ref_property_type_id: data.type,
    projects_edge_type_id: '',
    required_type: false,
    unique_type: false,
    updated_at: '',
...
/* check property name has or not */

export function addSuffixProperty(name: string, data: ItemMapping[]): string {
  const regex = new RegExp(`^${name}(?:_\\d+)?$`);
  const matchingItems = data.filter((item) => regex.test(item.property.name));

  if (matchingItems.length === 0) {
    return name;
  }

  return `${name}_${matchingItems.length}`;
}
...
export function processDataWithType(
  ...
  rowloop: for (let i = dataToMap.length; i > 0; i--) {
    ...
    for (const mapData of dataMapping) {
      ...
      if (mapData.property.default_property && !cellValue) {
        dataToMap.splice(index, 1);
        wrongValueCount.count++;
        wrongValueCount.emptyValue++;
        continue rowloop;
