...
const menuItems = (foldersList: FoldersList[], disabled: TDisableMenuItem): MenuItem[] =>
  [
    {
      key: '1',
      icon: <MoveTo />,
...
type Props = {
  ...
  moveToDisable: boolean;
};

/* forceSubMenuRender - set condiotn when popover become visible this will become true */
export const ProjectActionMenu = ({ foldersList, projectId, folderId, setIsDeleteModalOpen, moveToDisable }: Props) => {
  ...
  const disabledMenuItem = () => {
    const isShared = location.pathname === PATHS.SHARED;
    const isInGroupProjects = location.pathname.includes('groups/projects');

    const disabled = {
      moveTo: isShared || isInGroupProjects || moveToDisable,
      edit: isShared,
    };
    return disabled;
  };
  ...
  return (
    <>
      <Menu
        style={{ width: 256 }}
        mode="vertical"
        items={menuItems(foldersList, disabledMenuItem())}
