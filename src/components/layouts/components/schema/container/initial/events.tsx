...
import { ElementStyle, InitEvents, InitPerspectiveEvents, InitTemplateEvents } from '../../types';
...
/**
 * The Events are provides template switchers
 * @param graph
 */
export const initTemplateEvents: InitTemplateEvents = (graph: Graph) => {
  graph.on('node:click', async ({ node, e: { target } }) => {
    if (!target.closest('.x6-port-body')) {
      const openedCount = graph.getNodes().filter((n) => n.attrs?.body.allow).length;
