...
export const PropertyConnectionDetails = ({ isConnectionType }: Props) => {
  ...
  useEffect(() => {
    ...
    if (nodesList !== undefined) {
      const properties = nodesList?.find((n) => n.id === source)?.properties;

      const id = properties?.find((item) => item.default_property)?.id;

      if (properties) {
        form.setFieldValue('source_attribute_id', id);
      }
    }
  }, [form, source, target, inverse, nodesList]);
