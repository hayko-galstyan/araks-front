...
const StyledQuerySelectWrapper = styled.div`
  margin-top: 24px;
`;
const StatusErrorMessage = styled.p`
  color: ${COLORS.ALERT.RED};
`;

export const QueriesContent = ({ fieldName, propertyType, id, isMultiple }: ContentType) => {
...
export const PropertySection = ({ remove, fieldName, isVisualisation }: Props) => {
  ...
  const firstKeyStatus: string | null =
    queriesList[fieldName].query && Object.keys(queriesList[fieldName].query).length > 0
      ? queriesList[fieldName].query[Object.keys(queriesList[fieldName].query)[0]].status
      : null;
  const IsINvalid: boolean = firstKeyStatus === 'invalid'||queriesList[fieldName]?.status === 'invalid'? true:false
  return (
    <VerticalSpace size={0} style={ IsINvalid ? { borderColor: 'red', borderWidth: 2, width: '100%', borderStyle: 'solid' }:{width: '100%'}}>
      {queriesList[fieldName]?.color && (
        <div style={{ backgroundColor: queriesList[fieldName]?.color, height: '8px' }} />
      )}
      <StyledCollapse
        defaultActiveKey={!queriesList[fieldName].parent_id ? [] : ['1']}
        onChange={onChange}
        items={[
          {
            key: '1',
            ...
            children: (
              <>
                <>
                  {isVisualisation && (
                    <>
                    {firstKeyStatus === 'invalid' && (
                      <StatusErrorMessage>
                        Condition deleted. Please ensure all conditions are present.
                      </StatusErrorMessage>
                    )}
                    ...
                    <Form.Item
                      ...
                      rules={[
                        ...
                        () => ({
                          validator() {
                            if (
                              queriesList[fieldName].status === 'invalid' ||
                              (firstKeyStatus !== null && firstKeyStatus === 'invalid')
                            ) {
                              return Promise.reject('This field is invalid');
                            }
                            return Promise.resolve();
                          },
                        }),
