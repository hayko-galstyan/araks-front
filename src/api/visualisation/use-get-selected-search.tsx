...
export const useGetSelectedSearchData = (id: string, options: Options) => {
  const isPublicPage = useIsPublicPage();

  const urlSelectedSearch = isPublicPage ? GET_SEARCH_PUBLIC_DATA : GET_SEARCH_DATA;

  const url = urlSelectedSearch.replace(':project_id', id);

  return useMutation({
    mutationFn: (params: { id: string; action: string; _ts?: number }) => client.get(url, { params }),
